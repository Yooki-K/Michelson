...
public class RayReflect : MonoBehaviour
{
    ...
    [Header("光线宽度")] public float width=0.003f;
    ...
    private void Awake()
    {
        ...
        _lineRender.startWidth = width;
        _lineRender.endWidth = width;
    ...
    public string GetJingMian()
    {
      ...
      private List<Vector3> GetRenderPoints(Vector3 start, Vector3 dir, float dis, int times)
      {
        ...
        bool change = true;
        string lastName= GetJingMian();
        RaycastHit hit;
        while (dis > 0 && times > 0)
        {
            if (lastName!=null)
            {
                if (!Physics.Raycast(start, dir, out hit, dis,~LayerMask.GetMask(lastName)))
                    break;
            }
            else
            {
            ...
            else if (target.name.Contains("Maoboli"))
            {
